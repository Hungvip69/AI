...
function setup() {
  ...
  // Không khởi tạo webcam ở đây, sẽ khởi tạo trong onStart()
  // để tránh tạo hai lần và xử lý lỗi quyền truy cập tốt hơn
...
async function onStart() {
  ...
  try {
    ...
    // Kiểm tra và yêu cầu quyền truy cập webcam
    try {
      ...
      // Tạo video với p5 và gán stream đã có quyền
      video = createCapture(VIDEO);
      video.elt.srcObject = stream; // Gán trực tiếp stream đã được cấp quyền
      video.size(640, 480);
      video.hide(); // Chỉ dùng canvas để hiển thị
    } catch (camErr) {
      console.error('Lỗi camera:', camErr);
      setModelStatus('Không thể truy cập webcam. Hãy kiểm tra quyền truy cập camera trong trình duyệt và làm mới trang.');
      elStartBtn.disabled = false;
